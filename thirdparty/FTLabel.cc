  ...
  _height(0)
{
    ...
    GLUtils::loadShader(fontVertexShader, GL_VERTEX_SHADER, _programId);
    GLUtils::loadShader(fontFragmentShader, GL_FRAGMENT_SHADER, _programId);
...
void FTLabel::recalculateVertices(const char* text, float x, float y, int width, int height) {
    ...
    while(getline(input,line))
    {
        ...
        vector<string> words = splitText(line.c_str());
